"""The overarching experimental process"""
...
def run_exp(models, train_features, train_target, test_features, test_target, evals, rpt: int = 33, focused=False):
    """Runs the experiment, consisting of training the models then testing them and storing all results

    :param models: the list of models to run
    :param train_features: the training features
    :param train_target: the training target values
    :param test_features: the test features
    :param test_target: the test target values
    :param evals: the evaluation metrics
    :param rpt: the number of times to repeat the experiment
    :param focused: whether to use all the features, or just the 'focused' set of the most important

    Variables:
        train_all: all the training data
        test_all: all the test data

    :return: list of two items; the training data and the test data
    """
    train_all = []
    test_all = []
...
# Grid Search to find the best Hyperparameters -- choose your own model and parameters to test
...
# -------- EXP2 -------
# create the exp2 train/test datasets
...
all_exp2_dfs = run_exp(tuned_models,
                       exp2_f_train, exp2_t_train, exp2_f_test, exp2_t_test,
                       evals=['accuracy', 'roc_auc', 'average_precision', 'precision', 'recall', 'f1'],
                       rpt=1)
...
print('exp2 training means')
print(vis.build_comparison_tables(exp2_train_means))
print('exp2 test means')
print(vis.build_comparison_tables(exp2_test_means))


# ------- Additional RQ Exploration
# select the 'best model' based on visualizations -- I chose XGB based on lowest FN rate (wrong cured predictions)
...
##### use the one most effective model (XGB) trained on Exp1+2 data on June 1, 2020 to March 2021...
...
# XGBoost isn't affected, so test with two runs of LogisticRegression
# speed improvements only apparent when using Intel processors

import time

# sklearnex Logistic Regression
...
# normal sklearn Logistic Regression
